...
struct MediaPickerView: View {
    ...
    private func handleEdit() {
        guard let editHandler = config.editHandler else { return }
        
        let selectedMedia = viewModel.buildPickedMedia()
        
        Task {
            let editedMedia = await editHandler(selectedMedia)
            viewModel.applyEdits(editedMedia)
        }
    }
